...
class MgbaSink extends AudioWorkletProcessor {
  constructor() {
    ...
    this.port.onmessage = (e) => {
      ...
      if (msg && msg.flush) {
        this.pos = this.w;
        this.lastPost = this.w;
        this.port.postMessage(this.w);
        return;
      }
      if (msg && msg.rate) {
...
export async function load(config) {
    ...
    // ---------------------------------------------------------------- input
    let keyMask = 0;
    let touchMask = 0; // SoulGold extension: independent multi-touch input.
    let padMask = 0;
    let sentMask = -1;
    let codeToBit = new Map();
    const pushInput = () => {
        const mask = keyMask | padMask | touchMask;
        if (mask !== sentMask) {
            sentMask = mask;
    ...
    const tick = (now) => {
        ...
        let frames = 0;
        if (speed === 1 && audioCtx.state === 'running' && audioClockAdvancing(now)) {
            ...
            const due = Math.floor(((now - wallClockStart) / 1000) * framerate * speed);
            frames = due - wallClockFrames;
            wallClockFrames = due;
        }
        frames = Math.max(0, Math.min(MAX_FRAMES_PER_TICK * speed, frames));
        if (frames > 0) {
            runFrames(frames);
            renderFrame();
        }
    ...
    const instance = {
        ...
        loadBatterySave(bytes) {
            if (!(bytes instanceof Uint8Array) || ![512,8192,32768,65536,131072].includes(bytes.length)) throw new Error('Unsupported battery save size');
            const ptr = heapAlloc(mod, bytes);
            try { if (!mod._mgbawasm_sram_load(ptr,bytes.length)) throw new Error('Battery save import failed'); }
            finally { mod._free(ptr); }
        },
        setButtons(mask) { touchMask = mask & 1023; pushInput(); },
        releaseButtons() { touchMask=keyMask=padMask=0; pushInput(); },
        config: engineConfig,
        system,
        escMenuGroups() {
