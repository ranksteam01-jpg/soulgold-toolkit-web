...
export const manifest: EngineManifest = {
  ...
  description:
    'mGBA — the accuracy-focused Game Boy Advance emulator, which also runs Game Boy and Game Boy Color — compiled to WebAssembly. Loads .gba/.gb/.gbc cartridge images; the GBA BIOS is optional, since mGBA ships a high-level replacement.',
  artifacts: {
    wasm: 'mgba/mgba.wasm',
    js: 'mgba/mgba.js',
  },
  assets: [
    {
      ...
      description:
        'Game Boy Advance, Game Boy or Game Boy Color cartridge image. The right core is picked from the image itself, so the extension only has to be one mGBA recognizes.',
    },
    {
      key: 'bios',
      mountPath: '/bios.bin',
      ...
      accept: ['.bin', '.rom'],
      description:
        'Optional GBA BIOS dump (16 KiB). mGBA has a high-level replacement that runs virtually everything, so this is only needed for the boot animation and the handful of titles that read the BIOS directly.',
